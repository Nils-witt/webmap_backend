...
public class MapOverlayController {
    ...
    @PostMapping("")
    EntityModel<MapOverlayDto> newEntity(@RequestBody MapOverlay newEntity, @AuthenticationPrincipal User userDetails) {
    ...
    @PutMapping("{id}")
    EntityModel<MapOverlayDto> replaceEntity(@RequestBody MapOverlay newEntity, @PathVariable UUID id, @AuthenticationPrincipal User userDetails) {
        MapOverlay entity = this.repository.findById(id).orElseThrow(() -> new MapOverlayNotFoundException(id));
