...
import dev.nilswitt.webmap.base.ui.views.components.MapGroupEditDialog;
import dev.nilswitt.webmap.base.ui.views.components.MapGroupPermissionsDialog;
import dev.nilswitt.webmap.base.ui.views.filters.MapGroupFilter;
...
public class MapGroupView extends VerticalLayout {
    ...
    public MapGroupView(MapGroupRepository repository, AuthenticationContext authenticationContext, PermissionVerifier permissionVerifier, SecurityGroupRepository securityGroupRepository,
        ...
        this.createBtn = new Button("Create", event -> {
            ...
            if (!PermissionVerifier.hasAnyScope(user, SecurityGroup.UserRoleTypeEnum.MAPGROUP,
    ...
    private class MapGroupContextMenu extends GridContextMenu<MapGroup> {
        public MapGroupContextMenu(Grid<MapGroup> target) {
            ...
            this.addItem("Edit", event -> event.getItem().ifPresent(mapGroup -> {
                User user = currentUser();
                if (!permissionVerifier.hasAccess(user, SecurityGroup.UserRoleScopeEnum.EDIT, SecurityGroup.UserRoleTypeEnum.SECURITYGROUP)) {
            ...
            this.addItem("Delete", event -> event.getItem().ifPresent(mapGroup -> {
                User user = currentUser();
                if (!permissionVerifier.hasAccess(user, SecurityGroup.UserRoleScopeEnum.DELETE, SecurityGroup.UserRoleTypeEnum.SECURITYGROUP)) {
