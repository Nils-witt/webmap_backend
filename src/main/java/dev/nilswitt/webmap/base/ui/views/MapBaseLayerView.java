...
import dev.nilswitt.webmap.base.ui.views.components.MapBaseLayerEditDialog;
import dev.nilswitt.webmap.base.ui.views.components.MapBaseLayerPermissionsDialog;
...
public class MapBaseLayerView extends VerticalLayout {
    ...
    public MapBaseLayerView(MapBaseLayerRepository mapBaseLayerRepository, AuthenticationContext authenticationContext, SecurityGroupRepository securityGroupRepository,
                            ...
                            UserRepository userRepository, PermissionVerifier permissionVerifier) {
    ...
    private void configureCreateButton() {
        ...
        createBtn.addClickListener(event -> {
            ...
            if (!PermissionVerifier.hasAnyScope(user, SecurityGroup.UserRoleTypeEnum.MAPBASELAYER,
    ...
    private void configureGrid(MapBaseLayerRepository mapBaseLayerRepository) {
        ...
        menu.addItem("Permissions", event -> event.getItem().ifPresent(mapOverlay -> {
            User user = currentUser();
            if (!permissionVerifier.hasAccess(user, SecurityGroup.UserRoleScopeEnum.ADMIN, SecurityGroup.UserRoleTypeEnum.MAPBASELAYER)) {
        ...
        menu.addItem("Edit", event -> event.getItem().ifPresent(mapBaseLayer -> {
            User user = currentUser();
            if (!permissionVerifier.hasAccess(user, SecurityGroup.UserRoleScopeEnum.EDIT, SecurityGroup.UserRoleTypeEnum.MAPBASELAYER)) {
        ...
        menu.addItem("Delete", event -> event.getItem().ifPresent(entity -> {
            User user = currentUser();
            if (!permissionVerifier.hasAccess(user, SecurityGroup.UserRoleScopeEnum.DELETE, SecurityGroup.UserRoleTypeEnum.MAPBASELAYER)) {
